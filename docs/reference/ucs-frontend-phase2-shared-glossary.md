...
| Widget 1       | OAuth Plugin Quota Status       | `GET /ucs/quota` _(proposed)_       | Auth plugin token counts               | `planned` (Phase 2.5) |
| Widget 2       | AFT Code Health Status          | `GET /ucs/health` _(proposed)_      | AFT metrics (`E/W/D/U/C/T`)            | `planned` (Phase 2.5) |
| Widget 3       | Cache Diagnostics & Prompt Bust | `GET /ucs/cache` _(proposed)_       | `cache_read`, `full_bust`              | `planned` (Phase 2.5) |
| Widget 4       | AFT Live LSP Diagnostics        | `GET /ucs/diagnostics` _(proposed)_ | LSP diagnostic items                   | `planned` (Phase 2.5) |
